...
function Write-ReleaseGuideHtml {
    param(
        [string]$DestinationRoot,
        [string]$RelativePath
    )

    $sourcePath = Join-Path $repoRoot "docs\index.html"
    if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
        throw "Required documentation entry is missing: $sourcePath"
    }

    $destinationPath = Join-Path $DestinationRoot $RelativePath
    $guideHtml = Get-Content -LiteralPath $sourcePath -Raw -Encoding UTF8
    $guideHtml = $guideHtml.Replace("images/", "docs/images/")
    [System.IO.File]::WriteAllText($destinationPath, $guideHtml, [System.Text.UTF8Encoding]::new($false))
}
...
$releaseGuideHtmlRelativePath = "OK-Record-User-Guide.html"
...
Write-ReleaseGuideHtml -DestinationRoot $packageDir -RelativePath $releaseGuideHtmlRelativePath
...
$archiveRootEntries += $releaseGuideHtmlRelativePath
