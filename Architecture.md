...
Contract: UXP owns Photoshop API calls, panel lifecycle, DOM events, and status display. Critical recording/export failures and export completion keep the bottom selectable detail notice and also show a native blocking alert so users cannot miss the result. Scheduled capture treats Photoshop `executeAsModal` host-modal collisions, such as an active Free Transform operation, as transient host-busy state: recording stays active, no blocking failure alert is shown, and the capture is retried shortly. Active recording document continuity is classified before capture from the locked recording context, the current document context, and Photoshop's locked document-reference validity. A `close` notification triggers bounded reconciliation but is not itself proof that the locked document closed. Closing the locked document ends recording normally without a failure alert, including when closure occurs after scheduled-capture preflight or while document pixels are being read. A confirmed close request remains authoritative until the in-flight capture settles. Frame-capture execution reports its failure phase to the scheduled-capture owner: document-read failures caused by a confirmed close become normal termination, while native frame-commit failures remain visible recording failures. Switching to another open document suspends scheduled capture without ending the locked timeline; returning to the locked document starts a full new sampling interval, unless the user manually paused or stopped recording. The locked Photoshop document ID is supplied to frame pixel reads so a switch during capture cannot source pixels from another document. Changing the locked document's own path or project identity remains a recording failure; temporary host document unavailability receives a bounded retry and never reuses the start-recording save-first error. Pure JS domain modules own recorder state transitions, target-duration export profile math, path policy, settings parsing, and painting timer rules. Native addon calls route through a single service bridge before entering UI workflow code.
...
- `uxp/domain/recording-context.js` owns local PSD/PSB document identity, the OK-Record project output directory, document-scoped manual project-directory overrides, recording output context, active-session/current-document matching, and pure active-recording continuity classification. It intentionally ignores `activeDocument.saved` because that flag means "saved since last edit", not "has a local file path". Default recording roots are derived from the current saved PSD/PSB path. A manual project root applies only when the persisted `frameOutputDocumentKey` matches the current document key, so incremental PSD/PSB saves can continue one project only after the user explicitly selects that project folder for the current document; active recording writes still lock to the currently opened document identity to prevent accidental writes after a document switch. UXP supplies Photoshop document-reference validity to the pure continuity classifier and uses the same reconciliation result from close notifications, capture preflight, and pre-commit capture failure handling. `uxp/main.js` retains a confirmed close request until the in-flight capture settles and owns the single failure-notice decision, so closing an unrelated document cannot stop the active timeline, closing the locked document cannot be misreported as a capture failure, and a real native write failure is not hidden by a concurrent close.
