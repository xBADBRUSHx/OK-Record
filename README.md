...
普通用户请从 GitHub Releases 下载 `.ccx` 安装包：
...
- `OK-Record_20260530.ccx`: 轻量版。适合已经安装或愿意自己配置 FFmpeg 的用户。
- `OK-Record_20260530_with-ffmpeg.ccx`: 免配置版。内置 FFmpeg，适合希望下载后直接导出视频的用户。

macOS 版本需要在 Mac 上构建并完成真实 Photoshop 测试后再发布。
...
- Windows 用户优先使用 Release 页面提供的 `.ccx` 包。
- macOS 用户目前需要从源码构建并反馈测试结果。
